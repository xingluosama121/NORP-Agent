...
保存为 `hello.py` 运行，控制台打印 `[norpagent] frontend web listening on 127.0.0.1:8787`，浏览器访问该地址打开聊天界面。
...
```bash
norpagent-rescue list                     # ★ = 最后一次正常工作的快照
norpagent-rescue rollback --last-good     # 一键回退（下次启动自动应用）
norpagent --safe-mode                     # 安全模式启动（跳过全部插件）
```

### 人类救援（模型失效时手操工具）

模型宕机但工作区 / 沙箱 / 上下文库 / 任务队列还活着？人类直接接管
工具层（手动传入参数、读出原始结果），与模型走完全相同的执行路径：

```bash
norpagent-rescue tools                              # 全部 20 个内置工具清单
...
程序化：`norpagent.rescue_api`（`RescueToolEnvironment` / `RescueToolAPI`）。
详见手册 15.6。
...
- **完整开发手册**：`test/docs/DEVELOPER_MANUAL.md`——23 章 + 5 附录：架构与数据流、槽位表热插拔、nasyncio 事件循环（含「不依赖标准 asyncio」完整声明）、前端体系、钩子、安全、插件、嵌入式与超高并发部署、工作回退（15.6 人类救援手操工具 API）、FLOW 模块流程编排、内置组件深度剖析、Web UI 深度解析、性能设计与基准、迁移指南、FAQ、术语表、钩子负载速查、API 索引；另有英文完整副本 `DEVELOPER_MANUAL.EN.md`；
- **技术架构白皮书**：`test/docs/ARCHITECTURE_WHITEPAPER.md`——架构设计与技术分享（原理向）：设计哲学、分层架构、槽位系统、内核、nasyncio、钩子、安全、插件、可靠性、FLOW、性能实测与架构决策记录（ADR），另有同源 `.docx` 版本；
- **钩子速查**：`test/docs/hooks.md`；
- **回归测试**：`test/_verify_*.py` / `test/_smoke_*.py`——覆盖 nasyncio 迁移、取消语义、热挂载循环、槽位热插拔、嵌入式与并发、WebUI 冒烟、front.html 迁移、人类救援手操 API 等全部专项；`test/test_minimal_kernel_suite.py` 为最小内核 48 项全量套件。
